...
def get_cities():
    ...
    if 'city_like' in param_dict:
        ...
        query = db.session.execute(
            "SELECT cities.city_name, COUNT(cities.city_id) AS city_count \
                FROM COMPANIES JOIN CITIES ON companies.city_id=cities.city_id \
                    WHERE lower(cities.city_name) LIKE lower(:city_like) \
                        GROUP BY companies.city_id, cities.city_name \
                            ORDER BY city_count DESC", {"city_like": city_like})
    else:
